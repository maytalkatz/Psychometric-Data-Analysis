...
def generate_psychometric_matrices(df, sub_TH_L, sub_TH_R):
    ...
    for opto in unique_optoStims:
        ...
        bilateral_L_first_row = df_opto[(df_opto['stim_left'] == 0) & (df_opto['stim_right'] == 0.2)]
        bilateral_R_first_row = df_opto[(df_opto['stim_right'] == 0) & (df_opto['stim_left'] == 0.2)]
...
def fit_psychometric_curves(none_mats):
    """
    Processes and fits psychometric curves for each condition in the provided matrices using psignifit.
    For each condition, the function:
      - Uses a Weibull sigmoid model with a Yes/No (binary) option for fitting.
      - Sets up fit options where the baseline (or lower asymptote) is fixed via fit_opts.
      - Obtains fit parameters (fit_pars) that describe the curve's location, slope, and asymptotes.
      - Computes an exponential transformation:
            exp_values = exp(c * (log(x_vals_cond) - fit_pars[0]) / fit_pars[1])
        where 'c' is a constant derived from specific percentiles (0.05 and 0.95).
      - Calculates the final psychometric curve (psych_curve) in percentage,
        incorporating the fit parameters to scale and shift the sigmoid.
...
def plot_fit_results(fit_results, mouse_id, day_range, opto_name, sub_th_L, sub_th_R):
    """
    Plots the fitted psychometric curves using the fit_results.
    Creates a composite figure with two subplots for unilateral and bilateral conditions.
    The legend shows the Weibull fit (line) and threshold (vertical line).
    Additionally, a text box is added to each subplot displaying the trial counts for the unilateral and bilateral conditions.
    """
    ...
    # Left subplot: L Unilateral & Bilateral (R constant sub TH)
    ax = axes[0]
    max_left = 0
    if indiv_UL:
    ...
    # Add a text box with trial counts in the left subplot
    ...
    # Right subplot: R Unilateral & Bilateral (L constant sub TH)
    ax = axes[1]
    max_right = 0
    if indiv_UR:
    ...
    # Add a text box with trial counts in the right subplot
...
def run_processing(params):
    """
    Runs the data loading and processing pipeline with the given parameters.
    If an AssertionError occurs (e.g., insufficient data for psignifit to compute the threshold),
    a pop-up window informs the user and the GUI for variable input is restarted.
    """
    try:
...
def select_base_folder():
    """ First window: Select the base folder using a styled GUI. """
    root = tk.Tk()
